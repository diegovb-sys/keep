...
| **KEEP_CUSTOM_DATE_DAYS_FILTER**          | Specifies the number of days to display in the Feed and Incidents   | No       | all-time      | Positive integer|
| **KEEP_CUSTOM_DATE_PAUSE_REFRESH_FILTER** | Specify if the Feed/Incidents visualization is stopped or not       | No       | "false"       | "true", "false" |
